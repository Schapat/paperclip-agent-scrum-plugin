...
import { MemoryStorageAdapter, persistState, restoreState } from '../storage';
...
describe('Ticket-Lifecycle end-to-end', () => {
  ...
  it('übersteht einen Neustart mit vollständigem Verlauf', async () => {
    const task = createScrumTask({
      title: 'Login',
      description: '',
      refined: true,
    ...
    await persistState(adapter, state);
    ...
    // Kommentare, Entscheidungen und Nachrichtenprotokoll bleiben erhalten
